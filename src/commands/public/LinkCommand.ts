...
export class LinkCommand extends Command implements IRunnableCommand {
    public async run(): Promise<void> {
        if (this.bot.verifiedUsers[this.message.author.id]) {
            let message = `Hey ${(await this.bot.cubecraftForum.getUserInfo(this.bot.verifiedUsers[this.message.author.id])).username}! You already have your account linked, use \`${Bot.prefix}unlink\` to unlink your account.`;
